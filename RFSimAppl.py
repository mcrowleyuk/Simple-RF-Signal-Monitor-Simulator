from flask import Flask, jsonify, render_template, request, redirect, url_for, session
import socket
import threading

app = Flask(__name__)
app.secret_key = 'hello+!'  # Replace with a secure random key

# Shared data store for latest UDP message
latest_data = {'signal_strength': -100, 'frequency': 2400}

# --- UDP Listener Thread ---
def udp_listener():
    HOST = '127.0.0.1'
    PORT = 8082
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s.bind((HOST, PORT))
    print(f"[UDP] Listening on port {PORT}...")

    while True:
        data, addr = s.recvfrom(1024)
        message = data.decode().strip()
        print(f"[UDP] Received from {addr}: {message}")
        try:
            # Expecting format like "signal_strength,frequency"
            parts = message.split(' ')
            if len(parts) == 2:
                latest_data['signal_strength'] = int(float(parts[0]))
                latest_data['frequency'] = int(float(parts[1]))
                print(latest_data['signal_strength'])
                print(latest_data['frequency'])
                
        except Exception as e:
            print(f"[UDP] Error parsing message: {e}")

# --- Flask Routes ---
@app.route('/')
def home():
    if not session.get('authenticated'):
        return redirect(url_for('login'))
    return render_template('index.html')

@app.route('/rf')
def rf_data():
    return jsonify(latest_data)

@app.route('/login', methods=['GET', 'POST'])
def login():
    if request.method == 'POST':
        password = request.form['password']
        if password == app.secret_key:
            session['authenticated'] = True
            return redirect(url_for('home'))
        else:
            return render_template('login.html', error='Invalid password')
    return render_template('login.html')

@app.route('/logout')
def logout():
    session.pop('authenticated', None)
    return redirect(url_for('login'))

# --- Start UDP Listener in Background ---
if __name__ == '__main__':
    threading.Thread(target=udp_listener, daemon=True).start()
    app.run(host='127.0.0.1', port=5000, debug=True)